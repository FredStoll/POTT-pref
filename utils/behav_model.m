...
function [ALL,param] = behav_model(filename,perm,showfig)
...
if length(choice) > 100  %- trash sessions with less than 100 trials diff juice
    ...
    bins = 30;
    parfor b = 1 : height(T)-bins
        warning off
        disp(b/(height(T)-bins))
        mdl_bins = fitglm(T(b:b+bins,:),modelspec,'Distribution','binomial','Link','logit');
        ft_bins(b) = median(table2array(T(b:b+bins,'ft')));
        rt_bins(b) = median(table2array(T(b:b+bins,'rt')));
        resid(1,b) = mean(mdl.Residuals.Raw(b:b+bins));
    ...
    for b = 1 : sum(restr)-bins
        boundaries = diffIns_tr([b b+bins]);
