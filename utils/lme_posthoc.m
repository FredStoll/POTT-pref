function [pval,wald,thr,pval_adj] = lme_posthoc(lme,measures,area2test,fig2do)

emm = emmeans(lme,{measures});
eval(['emm.tab_post = emm.table.' measures ';']);
thr = .05;
%h = emmip(emm,'area');
clear pval wald
pval_all=[];
for ar1 = 1 : length(area2test)
    for ar2 = 1 : length(area2test)
        if ar1>ar2
            L_post = [ strcmp(emm.tab_post,area2test{ar1})' - strcmp(emm.tab_post,area2test{ar2})'];
