...
function fighandle = rasterPSTH_2bins_SUB(Unit,evt2take,evtName,cond,grp,pre,post,pathFig)
...
for c = 1 : length(ncd_sub)
    if trashCd(ncd_sub(c))~=1
        ...
        ciplot(psth_all(:,ncd_sub(c))-psth_all_sem(:,ncd_sub(c)),psth_all(:,ncd_sub(c))+psth_all_sem(:,ncd_sub(c)),timevec,colors(c,:),1);
    end
end
...
box on;
set(gca,'XTick',[],'FontSize',8,'color','w','xcolor',[1 1 1]);
...
for g = 1 : length(grp_sub(:,1))
    ...
    ciplot(psth_avg(takeit)-psth_avg_sem(takeit),psth_avg(takeit)+psth_avg_sem(takeit),mod(ncd(takeit),100),colo(g,:),1);
end
